...
class EspMainActivity : AppCompatActivity() {
    ...
    private fun initViews() {
        ...
        val btnAddDevice = findViewById<CardView>(R.id.btn_provision_device)
    ...
    var addDeviceBtnClickListener = View.OnClickListener {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            if (!isLocationEnabled) {
                askForLocation()
    ...
    private fun askForLocation() {
        ...
        // Set up the buttons
        builder.setPositiveButton(R.string.btn_ok) { dialog, which -> startActivityForResult(Intent(Settings.ACTION_LOCATION_SOURCE_SETTINGS), REQUEST_LOCATION) }
